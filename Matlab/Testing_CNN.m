...
% path1 = './testing_data/server_brainlab/GT_train_42x42_20190924_ON_OFF/';
...
for region_cnt = 1:region_num
    ...
    for i=1:5
        classout_hardware(i,region_cnt) = hex2sdec(class_out{i,region_cnt},32);
        fprintf('class[%d] = 0x%s, 0d%d\n', i, class_out{i,region_cnt},classout_hardware(i,region_cnt));
    end
    [~,predClassLabel] = max(hex2sdec(class_out(:,region_cnt),32));
    fprintf(fileID,'%d\n', predClassLabel);
end
...
classout_software = zeros(5, region_num); %5 classes
for i=0:region_num-1
%     classout_from_software(:,i+1) = load([output_path1,path2,'/',num2str(i),'/final_output.txt']);
    classout_software(:,i+1) = load([path1,path2,'/',num2str(i),'_final_output_OldWt.txt']);
end
if show_image
