...
save_avi_en = 0;
show_image = 0;%1;
%% weights/pointer/bias files (lavanya) from /home/Div6/research/lavanyay19/STEE_PROJ/vcs
% read text file
...
dfp_out_lyr2 = 'FC';
dfp_out_lyr3 = 'FC';
%% weights/pointer/bias files (sohan) from /media/project2/media/project2/soham/soham/KT_files/chip/hardware
% read text file
weights_mem1 = textread('./testing_data/server_brainlab/hardware/conv2d_1_weight_table.txt','%s');
weights_mem2 = textread('./testing_data/server_brainlab/hardware/separable_conv2d_1_weight_table.txt','%s');
weights_mem4 = textread('./testing_data/server_brainlab/hardware/dense_finetune_weight_table.txt','%s');
kernel_ptr_lyr1     = textread('./testing_data/server_brainlab/hardware/weights_conv2d_1_weight_pointer.txt','%s');
kernel_ptr_dw_lyr2  = textread('./testing_data/server_brainlab/hardware/weights_separable_conv2d_1depthwise_weight_pointer.txt','%s');
kernel_ptr_pw_lyr2  = textread('./testing_data/server_brainlab/hardware/weights_separable_conv2d_1pointwise_weight_pointer.txt','%s');
kernel_ptr_fc_lyr   = textread('./testing_data/server_brainlab/hardware/weights_dense_finetune_weight_pointer.txt','%s');
bias_lyr1 = textread('./testing_data/server_brainlab/hardware/conv2d_1_bias_values.txt','%s');
bias_lyr2 = textread('./testing_data/server_brainlab/hardware/separable_conv2d_1_bias_values.txt','%s');
bias_lyr3 = textread('./testing_data/server_brainlab/hardware/dense_finetune_bias_values.txt','%s');
% dfp_bias_lyr1   = textread('./testing_data/server_brainlab/hardware/dfp_bias.txt','%s');
% dfp_lyr1        = textread('./testing_data/server_brainlab/hardware/dfp_conv.txt','%s'); 
% dfp_bias_lyr2   = textread('./testing_data/server_brainlab/hardware/dfp_bias_layer_2.txt','%s');
% dfp_lyr2        = textread('./testing_data/server_brainlab/hardware/dfp_seperable_conv.txt','%s');
% dfp_bias_lyr3   = textread('./testing_data/server_brainlab/hardware/dfp_bias_layer_3.txt','%s');
% dfp_lyr3        = textread('./testing_data/server_brainlab/hardware/dfp_dense.txt','%s'); 
% dfp_out_lyr1 = 'FC';
% dfp_out_lyr2 = 'FC';
% dfp_out_lyr3 = 'FC';
%% weights/pointer/bias for debug
% read text file
weights_mem1 = num2str(ones(16,1)*1);% textread('./testing_data/server/22mar2020/conv2d_1_weight_table.txt','%s');
% weights_mem2 = num2str(ones(16,1)*0);% textread('./testing_data/server/26mar2020/separable_conv2d_1_weight_table.txt','%s');
...
bias_lyr1 = num2str(ones(6,1)*0);% textread('./testing_data/server/22mar2020/conv2d_1_bias_values_dbg.txt','%s');
...
dfp_bias_lyr1   = 'FC';% textread('./testing_data/server/22mar2020/dfp_bias.txt','%s');
dfp_lyr1        = 'FC';% textread('./testing_data/server/22mar2020/dfp_conv.txt','%s'); 
...
lyr1_ptr_addr_cnt = 0;  % the cnt is continous between layer1 layer2 dw and pw
...
for lyr3_ptr_cnt=1:49
    %data: 49*5*5=1225
    spi_config(xem, 81, hex2dec(        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*4)))));
    spi_config(xem, 82, hex2dec(strcat( dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*3))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*2))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*1))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*0))))));
    spi_config(xem, 79, hex2dec(        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*4+49*5)))));
    spi_config(xem, 80, hex2dec(strcat( dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*3+49*5))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*2+49*5))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*1+49*5))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*0+49*5))))));
    spi_config(xem, 77, hex2dec(        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*4+49*5*2)))));
    spi_config(xem, 78, hex2dec(strcat( dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*3+49*5*2))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*2+49*5*2))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*1+49*5*2))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*0+49*5*2))))));
    spi_config(xem, 75, hex2dec(        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*4+49*5*3)))));
    spi_config(xem, 76, hex2dec(strcat( dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*3+49*5*3))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*2+49*5*3))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*1+49*5*3))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*0+49*5*3))))));
    spi_config(xem, 73, hex2dec(        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*4+49*5*4)))));
    spi_config(xem, 74, hex2dec(strcat( dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*3+49*5*4))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*2+49*5*4))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*1+49*5*4))), ...
                                        dec2hex(hex2dec(kernel_ptr_fc_lyr(lyr3_ptr_cnt+49*0+49*5*4))))));
    %addr:0~49-1
...
fileID = fopen('HW_Predicted_Labels_GT_Data.txt','w');
fileID2 = fopen('SW_Predicted_Labels_GT_Data.txt','w');

% fprintf(fileID,'%1s %10s\n', 'expected', 'predicted');
...
% path1 = './testing_data/server_brainlab/20180717_Site2_4pm_6mm_01_HW_CNN_Test_data/';
path1 = './testing_data/server_brainlab/GT_1B2C_HR_TruckVanCombined/';
% path1 = './testing_data/server_brainlab/20180712_Site2_3pm_6mm_04_HW_CNN_Test_data/';
...
% img_pos = zeros(240*320,1);
% img_pos(1:320*10,1) = 1;
img_neg = load([path1,path2,'/',file_name_neg]);
% img_neg = zeros(240*320,1);
...
for i = 1:region_num*2
    ddata_rx(2*i-1) = rp(2*i)-1;%index starts from 0 in verilog
    ddata_ry(2*i-1) = rp(2*i+1)-1;
end
...
if save_gif_en 
    save_as_gif(img, 304, fig, 0.5, 'out.gif');
end
if save_avi_en
    frame = getframe(fig);       %// ??????????
...
classout_hardware = zeros(5,region_num);
...
for region_cnt = 1:region_num
    ...
    for i=1:5
        classout_hardware(i,region_cnt) = hex2sdec(class_out{i},32);
        fprintf('class[%d] = 0x%s, 0d%d\n', i, class_out{i,region_cnt},classout_hardware(i,region_cnt));
    end
    [~,predClassLabel] = max(classout_hardware);
    fprintf(fileID,'%d\n', predClassLabel);
end
...
%     classout_from_software(:,i+1) = load([path1,path2,'/',num2str(i),'_final_output_OldWt.txt']);
% end
%%dummy SW results
classout_software=zeros(5,1);
classout_software(GTLab(img+1)+1) = 10;
if show_image
    % visualize(img_pos, img_neg, rp, class_out);
    % visualize_v2(img_pos, img_neg, rp, class_out, path1,path2, t_hw,t_sw);
    results_show(img_pos, img_neg, rp, classout_hardware, classout_software);
    pause(0.2)
end

[~,predClassLabel_sw] = max(classout_software);
fprintf(fileID2,'%d\n', predClassLabel_sw);
